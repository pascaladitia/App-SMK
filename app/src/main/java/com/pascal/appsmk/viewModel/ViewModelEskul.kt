...
class ViewModelEskul(application: Application): AndroidViewModel(application) {

    var isEmpty = MutableLiveData<Boolean>()

    fun insertEskul(name: String, ttl: String, alamat: String, jabatan: String, tentang: String, imgPath: Uri) {
        if (name.isNotEmpty() && ttl.isNotEmpty() && alamat.isNotEmpty() && jabatan.isNotEmpty() && tentang.isNotEmpty()) {
            val storageRef = FirebaseStorage.getInstance().getReference("eskul")
            val databaseRef = FirebaseDatabase.getInstance().getReference("eskul").push()
    ...
    fun updateEskul(name: String, ttl: String, alamat: String, jabatan: String, tentang: String, key: String, imgPath: Uri) {
        if (name.isNotEmpty() && ttl.isNotEmpty() && alamat.isNotEmpty() && jabatan.isNotEmpty() && tentang.isNotEmpty()) {
            val storageRef = FirebaseStorage.getInstance().getReference("eskul")
            val databaseRef = FirebaseDatabase.getInstance().getReference("eskul")
